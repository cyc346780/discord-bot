...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!interaction.member.voice.channelId) {
      ...
      await interaction.channel.send({ content: `🎵｜<@${interaction.user.id}> 開啟/關閉單曲循環`, embeds: [embedMsg] })
        .then((message) => {
          setTimeout(() => {
            message.delete();
          }, 5000);
    ...
    const embedMsg = new MessageEmbed()
      .setColor('#2ECC71')
      .setTitle(queue.repeatMode === QueueRepeatMode.OFF ? '🔁｜關閉單曲循環' : '🔁｜開啟單曲循環');
